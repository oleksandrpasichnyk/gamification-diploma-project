...
function ScreenShake(camera) {
  return {
    ...
    update: function update() {
      if (this.enabled == true) {
        ...
        if (this._timestampEnd > now) {
          ...
          this.camera.position.copy(this._startPoint);
          this.enabled = false;
        }
      }
    },
    ...
    shake: function shake(vecToAdd, milliseconds) {
      this.enabled = true;
      this._timestampStart = Date.now();
      this._timestampEnd = this._timestampStart + milliseconds;
      this._startPoint = new THREE.Vector3().copy(this.camera.position);
      this._endPoint = new THREE.Vector3().addVectors(
        this.camera.position,
        vecToAdd
      );
    },

    computePosition: function computePosition(interval) {
      ...
      this.camera.position.lerpVectors(this._startPoint, this._endPoint, position);
    },
